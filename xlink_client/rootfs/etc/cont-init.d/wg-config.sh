...
if ! bashio::config.has_value 'gateway.host'; then
    bashio::exit.nok 'You need to specifiy the hostname of the remote gateway'
fi
gateway_host=$(bashio::config "gateway.host")
# Check if port is specified
if ! bashio::config.has_value 'gateway.port'; then
    bashio::exit.nok 'You need to specifiy the port of the remote gateway'
fi
gateway_port=$(bashio::config "gateway.port")
# Check if gateway public key is specified
if ! bashio::config.has_value 'gateway.public_key'; then
    bashio::exit.nok 'You need to specifiy the public key of the remote gateway'
fi
gateway_public_key=$(bashio::config "gateway.public_key")

gateway_allowed_ips="10.0.0.1/32"
gateway_keepalive=5

# Write gateway=peer to configuration file
{
    echo "[Peer]"
    echo "PublicKey = ${gateway_public_key}"
    echo "Endpoint = ${gateway_host}:${gateway_port}"
...
# todo:Add private key to client??

client_private_key=$(bashio::config "client.private_key")
# Check if client private key is specified
if ! bashio::config.has_value 'client.private_key'; then
    bashio::exit.nok 'You need to specifiy the client key'
fi
client_private_key=$(bashio::config "client.private_key")

client_addr="10.0.0.2/24"

# Write Client=interface to configuration file
{
    echo "[Interface]"
...
# Finish up the main server configuration
{
    ...
    # Custom MTU setting
    # bashio::config.has_value "gateway.mtu" && echo "MTU = ${mtu}"
